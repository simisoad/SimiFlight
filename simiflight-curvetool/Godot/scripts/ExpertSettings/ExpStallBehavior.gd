@tool
class_name ExpStallGeometry extends ExpertSetting
...
func setup_panel(physics_cfg: AeroPhysicsConfig) -> void:
	...
	# Reynolds
	SpinBoxSetupUtils.setup(stall_re_ref, 10000.0, 50000000.0, 10000.0, d.stall_re_ref,
		"Reference Reynolds Number.\nAt this Re, the Stall Angle Factor is nominal.\nLower Re reduces stall angle.")

	# LE Quality
	SpinBoxSetupUtils.setup(stall_le_quality_factor, 1.0, 50.0, 0.5, d.stall_le_quality_factor,
		"Leading Edge Quality.\nMultiplies the LE Radius to determine circulation efficiency.\nHigher = Better flow attachment.")


	SpinBoxSetupUtils.setup(stall_te_roundness_threshold, 0.001, 0.2, 0.001, d.stall_te_roundness_threshold,
		"Thickness Threshold.\nIf Trailing Edge thickness > this value, the back is considered 'Round'.\nAffects Backward Flight stall logic.")

	# --- FWD LIMITS ---
	SpinBoxSetupUtils.setup(stall_min_deg_fwd, 1.0, 45.0, 0.5, d.stall_min_deg_fwd,
		"Minimum Forward Stall Angle.", "Fwd Min")

	# The setup for Max must account for the current Min to prevent errors
	SpinBoxSetupUtils.setup(stall_max_cap_deg_fwd, d.stall_min_deg_fwd, 90.0, 0.5, d.stall_max_cap_deg_fwd,
		"Maximum Forward Stall Angle Cap.", "Fwd Max")

	# --- BWD LIMITS ---
	SpinBoxSetupUtils.setup(stall_min_deg_bwd, 0.1, 45.0, 0.1, d.stall_min_deg_bwd,
		"Minimum Backward Stall Angle.", "Bwd Min")

	SpinBoxSetupUtils.setup(stall_max_cap_deg_bwd, d.stall_min_deg_bwd, 90.0, 0.5, d.stall_max_cap_deg_bwd,
		"Maximum Backward Stall Angle Cap.", "Bwd Max")
	# Camber Shifts
	SpinBoxSetupUtils.setup(stall_camber_shift_sensitivity, 0.0, 100.0, 1.0, d.stall_camber_shift_sensitivity,
		"How much Camber shifts the Stall Angle range.\nHigh camber stalls later on top, earlier on bottom.")
	SpinBoxSetupUtils.setup(stall_camber_shift_min, -20.0, 0.0, 0.5, d.stall_camber_shift_min,
		"Max negative shift (degrees) due to camber.")
	SpinBoxSetupUtils.setup(stall_camber_shift_max, 0.0, 20.0, 0.5, d.stall_camber_shift_max,
		"Max positive shift (degrees) due to camber.")
	# --- SHARPNESS MULTIPLIERS ---
	SpinBoxSetupUtils.setup(stall_sharpness_fwd_mult, 0.1, 10.0, 0.1, d.stall_sharpness_fwd_mult,
		"Multiplier for the base sharpness in Forward flight. Default 1.0.", "Sharpness Mult Fwd")

	SpinBoxSetupUtils.setup(stall_sharpness_bwd_mult, 0.1, 10.0, 0.1, d.stall_sharpness_bwd_mult,
		"Multiplier for the base sharpness in Backward flight. Higher = snappier stall break.", "Sharpness Mult Bwd")
	# Connections
	stall_re_ref.value_changed.connect(func(v): _cfg.stall_re_ref = v; EventBus.reanalyze_requested.emit())
	stall_le_quality_factor.value_changed.connect(func(v): _cfg.stall_le_quality_factor = v; EventBus.reanalyze_requested.emit())
# 1. Update Model & Emit Signal for FWD
	stall_min_deg_fwd.value_changed.connect(func(v):
		d.stall_min_deg_fwd = v
	...
	stall_max_cap_deg_bwd.value_changed.connect(func(v):
		d.stall_max_cap_deg_bwd = v
		EventBus.stall_limits_bwd_changed.emit(stall_min_deg_bwd.value, v)
	)

	stall_te_roundness_threshold.value_changed.connect(func(v): _cfg.stall_te_roundness_threshold = v; EventBus.reanalyze_requested.emit())

	stall_camber_shift_sensitivity.value_changed.connect(func(v): _cfg.stall_camber_shift_sensitivity = v; EventBus.reanalyze_requested.emit())
	stall_camber_shift_min.value_changed.connect(func(v): _cfg.stall_camber_shift_min = v; EventBus.reanalyze_requested.emit())
	stall_camber_shift_max.value_changed.connect(func(v): _cfg.stall_camber_shift_max = v; EventBus.reanalyze_requested.emit())
	stall_sharpness_fwd_mult.value_changed.connect(func(v): _cfg.stall_sharpness_fwd_mult = v; EventBus.reanalyze_requested.emit())
	stall_sharpness_bwd_mult.value_changed.connect(func(v): _cfg.stall_sharpness_bwd_mult = v; EventBus.reanalyze_requested.emit())
