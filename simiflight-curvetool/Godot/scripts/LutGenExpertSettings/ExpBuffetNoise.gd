...
func _ready() -> void:
	SpinBoxSetupUtils.setup(buffet_base_shake_fwd, 0.0, 0.5, 0.0001, AeroPhysicsModel.buffet_base_shake_fwd,
		"Base noise amplitude (Forward).", "Base Shake Fwd")

	SpinBoxSetupUtils.setup(buffet_base_shake_bwd, 0.0, 0.5, 0.0001, AeroPhysicsModel.buffet_base_shake_bwd,
	...
	buffet_base_shake_fwd.value_changed.connect(func(v): AeroPhysicsModel.buffet_base_shake_fwd = v; EventBus.reanalyze_requested.emit())
