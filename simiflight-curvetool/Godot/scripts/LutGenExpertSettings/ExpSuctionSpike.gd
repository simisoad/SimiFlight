...
func _ready() -> void:
	...
	# --- Forward (Standard) ---
	SpinBoxSetupUtils.setup(spike_ref_radius_fwd, 0.00001, 1.0, 0.00001, AeroPhysicsModel.spike_ref_radius_fwd,
		"The LE Radius considered 'Standard' (100% capacity) for FWD flight. Usually 1.2% to 1.5% chord.")

	SpinBoxSetupUtils.setup(spike_max_capacity_fwd, 0.01, 10.0, 0.01, AeroPhysicsModel.spike_max_capacity_fwd,
		"Max multiplier for the suction spike in Forward flight.", "Fwd Max Capacity")

	SpinBoxSetupUtils.setup(spike_boost_mag_fwd, 0.0, 2.0, 0.01, AeroPhysicsModel.spike_boost_mag_fwd,
		"Magnitude of Lift Boost before stall (Forward).", "Fwd Boost Mag")

	SpinBoxSetupUtils.setup(spike_crash_mag_fwd, 0.0, 2.0, 0.01, AeroPhysicsModel.spike_crash_mag_fwd,
		"Magnitude of Lift Drop after stall (Forward).", "Fwd Crash Mag")

	SpinBoxSetupUtils.setup(recovery_thick_min_fwd, 0.01, 90.0, 0.01, AeroPhysicsModel.recovery_thick_min_fwd,
		"Recovery Angle for THIN wings (Forward). Hysteresis effect.", "Fwd Recov Thin")

	SpinBoxSetupUtils.setup(recovery_thick_max_fwd, 0.01, 45.0, 0.01, AeroPhysicsModel.recovery_thick_max_fwd,
		"Recovery Angle for THICK wings (Forward).", "Fwd Recov Thick")

	# --- Backward (Reverse/Bluff) ---
	SpinBoxSetupUtils.setup(spike_ref_radius_bwd, 0.00001, 1.0, 0.00001, AeroPhysicsModel.spike_ref_radius_bwd,
		"The LE Radius considered 'Standard' (100% capacity) for BWD flight. Lower this for sharp tails!", "Ref Radius Bwd")

	SpinBoxSetupUtils.setup(spike_max_capacity_bwd, 0.01, 10.0, 0.01, AeroPhysicsModel.spike_max_capacity_bwd,
		"Max multiplier for suction spike in Backward flight. Usually lower.", "Bwd Max Capacity")

	SpinBoxSetupUtils.setup(spike_boost_mag_bwd, 0.0, 2.0, 0.01, AeroPhysicsModel.spike_boost_mag_bwd,
		"Magnitude of Lift Boost before stall (Backward).", "Bwd Boost Mag")

	SpinBoxSetupUtils.setup(spike_crash_mag_bwd, 0.0, 2.0, 0.01, AeroPhysicsModel.spike_crash_mag_bwd,
		"Magnitude of Lift Drop after stall (Backward).", "Bwd Crash Mag")

	SpinBoxSetupUtils.setup(recovery_thick_min_bwd, 0.01, 90.0, 0.01, AeroPhysicsModel.recovery_thick_min_bwd,
		"Recovery Angle for THIN wings (Backward).", "Bwd Recov Thin")

	SpinBoxSetupUtils.setup(recovery_thick_max_bwd, 0.01, 45.0, 0.01, AeroPhysicsModel.recovery_thick_max_bwd,
	...
	# Fwd
	spike_ref_radius_fwd.value_changed.connect(func(v): AeroPhysicsModel.spike_ref_radius_fwd = v; EventBus.reanalyze_requested.emit())
	...
	# Bwd
	spike_ref_radius_bwd.value_changed.connect(func(v): AeroPhysicsModel.spike_ref_radius_bwd = v; EventBus.reanalyze_requested.emit())
