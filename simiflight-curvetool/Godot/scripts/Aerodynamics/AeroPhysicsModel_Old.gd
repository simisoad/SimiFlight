class_name AeroPhysicsModelOLD
extends RefCounted
...
static func _calc_stall_geometry(ctx: _CalcContext) -> void:
	...
	var target_slope = (2.0 * PI) * ctx.helmbold_multiplier * cos(deg_to_rad(ctx.c4_sweep_deg))
	ctx.effective_slope = lerp(base_slope, target_slope, ctx.vortex_weight)
	...
	var stall_bonus = ctx.p.vortex_stall_shift_max * ctx.final_intensity
	active_stall_base_deg += stall_bonus
...
static func _calc_base_lift(ctx: _CalcContext) -> void:
	...
	# Use the synchronized phase shift
	...
	# Apply phase shift to the Sine wave
...
static func _calc_base_lift_old(ctx: _CalcContext) -> void:
	# 1. Potential Component
	ctx.cl_potential = ctx.effective_slope * (ctx.effective_alpha - ctx.used_alpha0)
	...
	var thickness_damping = clamp(1.0 - (ctx.thickness * ctx.p.plate_thickness_damping_threshold), 0.8, 1.0)
	var cl_plate = ctx.p.cd_max * sin(2.0 * (ctx.alpha_rad - plate_phase_shift)) * current_scale * thickness_damping
...
static func _calc_suction_spike(ctx: _CalcContext) -> void:
	...
	if not ctx.is_forward and active_radius <= 0.0001:
		active_radius = (ctx.geo.te_openness * ctx.geo.te_openness) / 0.1

	active_radius = min(active_radius, 0.005)
	var raw_suction = sqrt(active_radius / ctx.p.spike_ref_radius_fwd)
	var suction_capacity = clamp(raw_suction, 0.0, p_capacity) * (1.0 - ctx.bluff_factor)
...
static func _calc_vortex_lift_integrated(ctx: _CalcContext) -> void:
	...
	ctx.final_cd += ctx.cl_vortex * tan(alpha_abs) # Vortex drag component
...
static func _calc_drag(ctx: _CalcContext) -> void:
	...
	cd0_profile += (ctx.p.drag_base_factor * ctx.passive_bluff_width)
	cd0_profile += (0.8 * max(0.0, ctx.active_bluff_width - 0.045))
	...
	var cd_wave = 0.0
	# Critical Mach decreases with thickness and lift
	var m_crit = clamp(0.9 - (ctx.thickness * ctx.p.drag_mcrit_thick_factor) - (abs(ctx.blended_cl) * 0.1), 0.6, 0.95)

	if ctx.mach > m_crit:
		# Wave drag factor is very sensitive to thickness
		var cd_wave_peak = ctx.p.drag_wave_factor * pow(ctx.thickness, 1.5) # Adjusted exponent for better scaling
		var ratio_mach = clamp((ctx.mach - m_crit) / (ctx.p.drag_wave_peak_mach - m_crit), 0.0, 1.0)

		if ctx.mach < ctx.p.drag_wave_peak_mach:
			# Rapid rise towards Mach 1.05
			cd_wave = cd_wave_peak * sin(ratio_mach * PI / 2.0)
		else:
			# Gradual decay in supersonic flow
			cd_wave = cd_wave_peak * (ctx.p.drag_wave_peak_mach / ctx.mach)

	# 4. Induced Drag (Sigma-Weighted)
	var cd_induced = 0.0
	if ctx.config.is_finite_wing and not ctx.config.is_bet_mode:
		var ar = max(ctx.config.aspect_ratio, 0.1)
		var raw_induced = pow(ctx.cl_potential * ctx.sigma, 2) / (PI * ar * ctx.config.oswald_efficiency)
		cd_induced = min(raw_induced, ctx.p.cd_max * ctx.p.induced_drag_cap_factor)

	# 5. Attached Flow Sum
	var cd_attached = cd0_profile + cd_induced + cd_wave

	# 6. Plate Drag (Separated Flow)
	var plate_mult = lerp(1.0, 0.92, clamp((ctx.mach - 1.5) / 8.5, 0.0, 1.0)) if ctx.mach >= 1.0 else 1.0

	# --- PHASE SHIFTED DRAG PEAK ---
	# We use (alpha - phase_shift) so the peak moves with the camber
	var cd_plate = (ctx.p.cd_max * plate_mult) * pow(sin(ctx.alpha_rad - ctx.plate_phase_shift), 2)

	# 7. Final Blending
	# Vortex drag (from _calc_vortex_lift_integrated) is already in ctx.final_cd
	ctx.final_cd += (ctx.sigma * cd_attached) + ((1.0 - ctx.sigma) * cd_plate)

	# 8. Sanity Cap (Allowing for Transonic Peak)
	# We allow the drag to exceed cd_max slightly during the Mach 1.0 transition
	var transonic_bonus = cd_wave * 0.5
	var absolute_max = ctx.p.cd_max * (1.0 + transonic_bonus)
	ctx.final_cd = min(ctx.final_cd, absolute_max)
...
static func _calc_drag_old(ctx: _CalcContext) -> void:
	...
	var m_crit = clamp(0.9 - (ctx.thickness * ctx.p.drag_mcrit_thick_factor) - (abs(ctx.blended_cl) * 0.1), 0.6, 0.95)
	if ctx.mach > m_crit:
		var cd_wave_peak = ctx.p.drag_wave_factor * pow(ctx.thickness, 2.0)
		var ratio_mach = clamp((ctx.mach - m_crit) / (ctx.p.drag_wave_peak_mach - m_crit), 0.0, 1.0)
	...
	# 5. Plate Drag
	var plate_mult = lerp(1.0, 0.92, clamp((ctx.mach - 1.5) / 8.5, 0.0, 1.0)) if ctx.mach >= 1.0 else 1.0
	var cd_plate = (ctx.p.cd_max * plate_mult) * pow(sin(ctx.alpha_rad), 2)

	# 6. Final Blending
	ctx.final_cd += (ctx.sigma * (cd0_profile + cd_induced + cd_wave)) + ((1.0 - ctx.sigma) * cd_plate)

	# 7. Sanity Cap
	ctx.final_cd = min(ctx.final_cd, ctx.p.cd_max * (1.0 + cd_wave))
...
static func calculate_finite_wing(cl_2d: float, cd_2d: float, aspect_ratio: float, efficiency: float, sweep_deg_user: float, taper: float, sweep_loc: float) -> Dictionary:
	...
	var multiplier = 1.0 / (sqrt(1.0 + pow(a0 / (PI * ar_eff), 2.0)) + (a0 / (PI * ar_eff)))
	var cl_3d = cl_2d * multiplier * cos_s
	var cd_induced = (cl_3d * cl_3d) / (PI * aspect_ratio * efficiency)
	return {"cl": cl_3d, "cd": cd_2d / max(cos_s, 0.5) + cd_induced, "sweep_c4": sweep_c4}
